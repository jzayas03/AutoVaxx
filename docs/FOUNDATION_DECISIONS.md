...
| 13. AI provider architecture | **REVISED FOR PLANNING** | Depend on a narrow `LocalAiProvider`; keep Ollama synthetic-evaluation-only and require a private-IPC, app-owned Windows worker for any patient-bearing use under [ADR-0010](adr/0010-use-a-private-ipc-owned-worker-for-patient-bearing-local-ai.md). | Isolates API drift, removes the unauthenticated local HTTP surface, and gives Rust enforceable lifecycle/resource ownership while AI remains optional and proposal-only. | No. | Worker/runtime/model license, AppContainer/GPU compatibility, hardware, packaging, and deployment hardening review before PHI. | Authenticated facility inference service or reviewed in-process runtime through a new ADR. |
...
| 22. Network policy | **DECIDED** | Default deny; patient-bearing AI has no network path, while speech may use only its separately controlled local boundary. Signed software/content import is offline; PREIS and external egress remain disabled. | The safest local-first baseline is no ambient network path for patient data. | No. | IT/security approves update and later destination allowlists. | Online signed updates and PREIS egress adapters. |
...
| 13. AI provider | Private IPC, no worker network capability, and no tools limit disclosure/action. | Suggestions remain nonauthoritative. | Worker/model/license/sandbox review before PHI. | Optional runtime can be unavailable without blocking work. | Stable provider contract, typed errors, deadlines, replaceable adapters. |
...
**Status: REVISED FOR PLANNING on 2026-09-05.** The domain depends on `LocalAiProvider`, never on Ollama-specific types. [ADR-0010](adr/0010-use-a-private-ipc-owned-worker-for-patient-bearing-local-ai.md) supersedes the earlier patient-bearing runtime topology but authorizes no implementation or real PHI.
...
**Options and tradeoffs.** A separately provisioned Ollama service is quickest but leaves an unauthenticated local HTTP and browser-origin surface outside AutoVaxx lifecycle control. An in-process runtime removes that endpoint but puts native crashes, memory exhaustion, and cancellation inside the trusted application. A private-IPC child worker adds sandbox, packaging, and cold-start work but lets Rust own authority, resources, and termination while preserving manual fallback.

**Recommendation.** Keep the fake-first provider contract. Use Ollama only for ephemeral synthetic evaluation. Patient-bearing product work must use an app-owned worker with no listening socket, an AppContainer with no network capability, and a no-breakaway Windows Job Object. A minimal llama.cpp-based worker is the first candidate, not an approved binary or model. If the required isolation is incompatible with the selected runtime or representative hardware, ship with local AI unavailable.

**Conceptual contract.** The provider reports health/readiness, verified worker-package and private-transport identity, available model descriptors/digests, capabilities (including schema-constrained output), and performs a bounded extraction request returning untrusted structured proposals with provider/model/prompt-template/schema/decoding provenance. It supports an absolute wall-clock deadline, cooperative cancellation, owned-process hard termination, and typed unavailable, timeout, hard-terminated, process-termination-failed, malformed-output, unsupported-capability, out-of-memory, provider-disk-full, resource-limit, model-digest-mismatch, and policy-denied errors. It has no domain-write, workflow, file, credential, registry, or arbitrary tool capability.

The developer evaluation `OllamaProvider` remains outside the product. The first product candidate is an owned-worker adapter using bounded, versioned anonymous-pipe frames and a pinned grammar-capable runtime. Provider differences remain inside adapters; neither HTTP servers nor generic tool APIs enter the product contract.

**Runtime ownership.** Rust creates the product worker suspended with AppContainer security attributes, assigns its Job Object before resume, passes only explicitly inherited private-pipe handles, and provides no network or general writable filesystem capability. AutoVaxx does not silently download a runtime/model or place patient content in arguments, environment variables, logs, or generic temporary paths. Cooperative cancellation is followed by Job Object termination and verified process-tree exit at the absolute deadline. Failed termination disables assistance until clean application restart.

**Identity, authority, and failure.** Verify the signed/hashed worker and model package against the approved manifest before the first patient-bearing assist session. The frontend cannot contact or launch the worker. The worker has no application, database, filesystem-tool, credential, registry, PREIS, or network authority. Timeout, cancellation, malformed output, model mismatch, and resource failure return to manual entry, do not mutate drafts, and never relax deterministic validation. Out-of-memory and isolated worker disk-full errors bypass retry/repair; unknown/shared-volume or clinical persistence exhaustion remains a blocking integrity failure. No cloud AI API may receive PHI.
...
**Recommendation.** Permit no patient-bearing AI network path. Require the ADR-0010 private-IPC owned worker for AI, retain the existing validated-local-provider controls for speech, and add external categories independently under the policy below.

Phase 1 and the first documentation slice have no external network requirement. Rust owns egress policy; React/Tauri capabilities and CSP do not expose generic HTTP. Patient-bearing AI has no network capability or listening endpoint. Speech may use a controlled child process or a separately validated loopback service under the existing speech controls. DNS names, redirects, proxies, wildcard listeners, and user-configured remote AI endpoints are rejected.
...
- AI/speech locality: patient data never goes to cloud AI/speech; patient-bearing AI uses only the private-IPC owned-worker boundary in ADR-0010, while any speech runtime requires its separately approved controlled-local-process boundary.
...
- Approve software/content update channels, signing keys, model/runtime source and checksum policy, owned-worker sandbox controls, speech-provider locality controls, and any future allowlisted destination.
...
11. Permit no external egress in Phase 1. AI provider tests enforce the private-IPC owned-worker design with no network/listening capability; speech provider tests enforce the existing controlled-child or validated-loopback boundary and reject remote endpoints, redirects, and proxies. The deterministic manual path works with all providers absent and network disabled.
...
- [Ollama authentication](https://docs.ollama.com/api/authentication), [Microsoft AppContainer isolation](https://learn.microsoft.com/en-us/windows/win32/secauthz/appcontainer-isolation), [Microsoft Job Objects](https://learn.microsoft.com/en-us/windows/win32/procthread/job-objects), [llama.cpp](https://github.com/ggml-org/llama.cpp), and [whisper.cpp server](https://github.com/ggml-org/whisper.cpp/blob/master/examples/server/README.md)

...
**Conceptual contract.** The provider reports health/readiness, local endpoint identity, available model descriptors, capabilities (including schema-constrained output), and performs a bounded extraction request returning untrusted structured proposals with provider/model/schema/version metadata. It supports deadline/cancellation and typed unavailable, timeout, malformed-output, unsupported-capability, resource-limit, and policy-denied errors. It has no domain-write, workflow, file, credential, registry, or arbitrary tool capability.
...
**Runtime ownership.** The initial MVP connects to a separately installed, facility-provisioned Ollama runtime. AutoVaxx does not download models, launch an elevated service, edit provider configuration, or silently pull a model while a patient is open. A later packaging decision may manage a pinned child process after supply-chain, licensing, patching, sandbox, resource, and support review.

**Locality and failure.** Resolve and validate the destination as IP loopback on every patient-bearing request; reject remote names, proxies, redirects, wildcard binds, and cloud endpoints. The frontend cannot contact the provider. Timeouts/cancellation return to manual entry, do not mutate drafts, and never relax deterministic validation. No OpenAI, Anthropic, Google, or other cloud AI API may receive PHI.
...
Persist only: assist-session ID; purpose/source type; provider and model identifier/version/hash when available; prompt-template/schema version (not prompt content); time; deterministic validation outcome; target field names; reviewer identity/time; accepted/rejected/cancelled disposition; and references to the resulting ordinary clinical revision. Accepted clinical values exist once in that revision; rejected values are not retained. Confidence may be retained only as nonclinical provenance tied to the decision, never as clinical certainty.
...
**Options and tradeoffs.** In-memory/pipe audio minimizes residue but may be harder with some runtimes. Private temporary files are compatible but need crash cleanup. Retained recordings aid replay but create a major new record class. The MVP uses memory/pipe when practical and rigorously temporary files otherwise, with no retention feature.

**Recommendation.** Prefer bounded in-memory/pipe transfer to a controlled whisper.cpp process. If a file is unavoidable, use PCM/WAV or another explicitly validated format in an application-private temporary directory with restrictive ACLs, a random nonidentifying filename, size/duration limits, and no path in logs or process arguments. Delete it after transcription on success, failure, cancellation, timeout, session lock, or logout. At startup, remove stale AutoVaxx-owned temporary audio after validating exact ownership/path and record only a non-PHI cleanup result.
